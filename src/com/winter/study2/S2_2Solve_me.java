	...
	public static void main(String[] args) {
		...
		while (flag) {
			...
			case 3:
				...
				// 이차원 배열 쓸일이 잘없음
			case 4: // 퀵정렬
				for (int i = 0; i < avgs.length; i++) { // 나머지애들도 같이 움직여야 함.. --> 어려우면 성적만 정리해서 출력
					for (int j = 0; j < avgs.length - 1; j++) {
						if (avgs[j] > avgs[j + 1]) {
							int temp = totals[j];
							totals[j] = totals[j + 1];
							totals[j + 1] = temp;
...
//							int temp7 = schoolnum[j];
//							schoolnum[j] = schoolnum[j + 1];
//							avgs[j + 1] = temp7;
						}
					}
				}
				...
				// 큰 순서대로 출력 4부터 0까지~**
				for (int i = avgs.length - 1; i >= 0; i--) { // 배열을 큰 순서대로 출력하기 위해 역순으로 출력
					System.out.println("순번\t이름\t국어\t수학\t영어\t총점\t평균");
					System.out.print(schoolnum[i] + "\t");
					System.out.print(name[i] + "\t");
					System.out.print(score[i] + "\t");
